...
路由不仅仅是 URL 跳转，它是单页应用（SPA）的骨架。Vue Router 4 的设计更加契合 Composition API，同时也提供了强大的动态路由和导航守卫机制。
...
**关键点**：`addRoute` 不会立即触发新的导航，所以在添加完路由后，通常需要 `next({ ...to, replace: true })` 来重新触发一次导航，确保新路由生效。
...
SPA 的通病是切换页面后滚动条位置不变。我们可以通过 `scrollBehavior` 修复这个问题。
...
掌握了路由的动态控制和守卫机制，我们就可以构建出安全、流畅的大型应用。下一篇，我们将深入 **Pinia**，看看它如何重新定义了 Vue 的状态管理。
