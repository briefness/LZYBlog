...
只要想做那种"随着滚动缩放头部"或者是"复杂嵌套滚动"的效果，**Slivers** 为必经之路。
...
### Sliver 复杂度 vs CustomScrollView 简洁性

`SliverList`、`SliverGrid` 等原生命令式 API 灵活，但写法繁琐——需要手动计算索引、管理 `ScrollController`。

`CustomScrollView` 封装了 Sliver 协议，组合多个滚动组件时更简洁。代价是：所有子组件必须遵循 Sliver 协议，混用普通 Box Widget 需要 `SliverToBoxAdapter`，增加嵌套层级。

### SliverKeepAlive vs AutomaticKeepAlive

`KeepAlive` 机制需要状态保持的 Item 实现 `AutomaticKeepAliveClientMixin` 并返回 `wantKeepAlive = true`。问题是：这个 Mixin 依赖 `KeepAliveNotification` 机制，如果父组件不是 `AutomaticKeepAliveClientMixin`，KeepAlive 就不会生效。

替代方案是 `SliverKeepAlive`，但需要自己管理哪些 Sliver 处于活跃状态，增加了状态管理的复杂度。
...
### 1. 嵌套 ListView 导致"Vertical viewport was given unbounded height"

`ListView` 本身基于 Sliver 协议。如果在外层已经是一个滚动视图，内层再放一个 `ListView`，会出现高度冲突报错。

**解法**：使用 `CustomScrollView` + `SliverList` 替代嵌套的 `ListView`。

### 2. SliverAppBar 展开高度不生效

`SliverAppBar` 的 `expandedHeight` 需要配合 `flexibleSpace` 使用。如果只设置了 `expandedHeight` 但 `flexibleSpace` 为空，AppBar 不会展开。

**解法**：确保 `flexibleSpace: FlexibleSpaceBar(...)` 存在。

### 3. cacheExtent 设置过大导致内存占用飙升

`cacheExtent` 默认 250px，会预渲染屏幕外的区域。如果列表项很复杂（包含图片、视频），大量预渲染会占用大量内存。

**解法**：根据 Item 开销调整 `cacheExtent`。纯文本可适当增大；含媒体资源建议保持默认值或减小。

### 4. KeepAlive 状态丢失

当 Item 滑出屏幕时，Element 一般会被销毁。若需保留状态（比如输入框里的字），可给该 Item 混入 `AutomaticKeepAliveClientMixin` 并返回 `wantKeepAlive = true`。但如果 Tab 切换导致整个 CustomScrollView 被销毁，KeepAlive 状态也会丢失——需要在外层 PageView 或 TabBarView 中单独处理。

**解法**：使用独立的状态管理（如 Provider/Riverpod）而非依赖 KeepAlive。
