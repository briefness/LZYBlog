...
## 5. Manim 动画：异步网络请求完整时序

**动画文件**：[animations/05_async_flow_animation.py](animations/05_async_flow_animation.py)

```bash
# 运行动画
manim -pql animations/05_async_flow_animation.py AsyncFlow
```

动画内容：
1. 节点依次亮起：Page → wx.request → Backend Server
2. 箭头动态连线：HTTP 请求发出，等待响应
3. 回调处理阶段：数据解析 → 错误判断 → 状态码检查
4. setData 阶段：数据回传 → 页面重新渲染
5. 强调 Promise 封装后的简洁调用模式
