...
1.  **Role (角色身份)**：定基调。是严肃的律师，还是搞怪的小丑？
2.  **Skills (能力范围)**：列出具体能力。会画图？会写代码？会查天气？
3.  **Constraints (限制条件)**：设围栏。不许撒谎，不许长篇大论，必须用中文。
4.  **Workflow (执行流程)**：SOP（标准作业程序）。第一步问什么，第二步做什么。
...
2. **给示例 (Few-Shot)**：AI 模仿能力极强。如果要让它输出 JSON 格式，直接提供 JSON 示例，比说“请输出 JSON”管用 10 倍。
...
下一篇，将给这位"员工"发几本专业书 —— **知识库 (Knowledge Base) 与 RAG 原理**，让 Bot 学会那些仅凭预训练数据无法掌握的领域知识。
