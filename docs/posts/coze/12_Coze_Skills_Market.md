...
在 Coze 2.0 中，创建 Skill 不再需要从头开始。我们既可以用 **Vibe Coding** 一句话生成，也可以将现有的 Workflow 封装。

### 3.1 方式一：Vibe Coding 自动生成 (实战 A股分析)
正如前面的逻辑图所示，我们来做一个 **“A股分析 Skill”**。

1.  **切换到 Skill 视图**：在 Coze IDE 顶部标签栏选择 **Skill**。
2.  **输入 Prompt**：
    > “我想实现一个A股分析的技能。输入股票代码（如 600519），先获取腾讯财经的实时价格，再获取雪球网的最近 5 条热评，最后分析当前的情绪是看多还是看空。”
3.  **自动构建**：
    Coze 会自动识别意图，从商店里把你需要的 `Get_Stock_Price` 和 `Get_Comments` 工具找出来，并自动配置好参数。
    *   *Input*: `stock_code`
...
如果你之前已经写好了一个复杂的 `News_Digest` (新闻早报) 工作流，也可以把它变成 Skill。
...
4.  **躺赚**：一旦审核通过，全球的 Developer 都能搜索并集成你的 Skill。
