...
### 特别说明：Coze 2.0 IDE 模式 (代码开发版)
如果你是通过 **Vibe Coding** 或 **IDE 模式** 开发的 Agent（如你的界面左侧有文件目录树），那么你右上角的按钮可能是黑色的 **“部署” (Deploy)**。
*   **经典模式 (Classic)**：点击 **“发布”** -> 选择渠道（商店/微信）。
*   **IDE 模式 (Pro)**：
    1.  先点击 **“部署” (Deploy)**：编译代码并启动云端服务容器。
    2.  部署成功后，再在 **“触发器 (Trigger)”** 或 **“渠道 (Channel)”** 配置页中，将这个服务连接到对应的 IM 渠道。
    3.  即：**IDE 模式下，部署是发布的前置动作。**
