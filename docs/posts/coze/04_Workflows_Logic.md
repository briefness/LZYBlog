...
若直接将这段 Prompt 发送给 Chatgpt，它可能会因为上下文中途断掉或者搜索内容太多而处理得一塌糊涂。
...
**工作流复杂度 vs 可维护性：甜蜜点在哪里？**

工作流的初衷是让复杂任务变得可管理。但节点越多，连线越密，维护成本指数级上升。
...
每增加一个节点，调试时要考虑的边界情况就多一层。如果工作流里的分支逻辑超过 3 层，建议重新思考架构。

**分支逻辑爆炸：if-else 的代价**

工作流里的"选择器"（if-else）节点看似简单，实际上是维护噩梦：
...
如果每个分支都有特殊处理逻辑，总测试用例数 = 2^n（n = 分支层数）。

**调试困难：黑盒里的断点**

工作流执行失败时，定位问题很难。Coze 不会告诉你"具体是哪一步"出了问题，只会说"执行失败"。

如果某个 LLM 节点的输出不符合预期，需要手动测试prompt是否能work。这个过程没法在工作流里直接调试。

## 常见坑点

### 1. 工作流节点之间的数据类型不匹配

**现象**：节点 A 输出 `data`（字符串），节点 B 却期望 `data.items`（数组）。

**原因**：Coze 节点之间的数据传递需要显式配置，不是自动推断。

**解法**：在每个节点配置里明确检查输入变量的类型和结构。用 `console.log` 或"代码节点"做类型转换。

### 2. LLM 节点输出格式不稳定

**现象**：工作流需要 JSON 输出，但 LLM 有时返回带 markdown 代码块的文本。

**原因**：LLM 本身不保证输出格式。它可能返回：
```json
{"key": "value"}
```
或者
...
**解法**：在 Prompt 里明确要求输出格式，并在后续节点加一个"代码节点"用正则提取 JSON。

### 3. 循环节点导致死循环

**现象**：工作流一直在跑，不结束。

**原因**：循环的退出条件永远为 true。比如：
```
如果列表长度 > 0，继续循环
但循环内部没有移除元素
```

**解法**：仔细检查循环的"终止条件"和"更新条件"。可以在循环节点旁边加一个日志节点输出当前状态。
