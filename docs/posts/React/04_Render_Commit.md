...
在 React 18 之前，一旦“渲染”开始，就必须一口气跑完。如果组件树很大，浏览器就会卡死。

**React 18 引入了并发渲染 (Concurrent Rendering)。**

现在的渲染过程是**可中断的**。React 就像一个聪明的多任务操作系统：
1.  开始渲染组件 A。
2.  突然用户点击了鼠标（高优先级事件）。
3.  React **暂停**渲染组件 A，先去响应鼠标点击。
4.  处理完点击后，再回来继续渲染组件 A（或者丢弃之前的成果重新开始）。

这就意味着：**渲染阶段可能被多次调用，或者被废弃。**因此，组件函数必须保持**纯粹**（Pure），绝对不能在渲染期间产生副作用（比如发网络请求），否则会导致副作用执行多次，引发 Bug。
...
4.  **渲染可中断**。在 React 18+，渲染阶段不再是阻塞的，它随时可能暂停，所以必须保证渲染函数无副作用。
