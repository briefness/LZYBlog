...
```mermaid
flowchart TB
    Hook["🧩 useOnlineStatus()<br/>useState + useEffect"]
    Hook -->|"独立实例 A"| CompA["StatusBar<br/>isOnline = true"]
    Hook -->|"独立实例 B"| CompB["SaveButton<br/>isOnline = true"]
    Note["💡 共享逻辑蓝图<br/>各自独立状态"]

    style Hook fill:#E3F2FD
    style CompA fill:#E8F5E9
    style CompB fill:#E8F5E9
```
