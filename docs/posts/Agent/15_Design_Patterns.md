...
| 你的需求 | 选择 |
|---------|------|
| 5 个以内的工具，单轮或简单多轮 | PydanticAI |
| 需要结构化输出 + 类型安全 | PydanticAI |
| 简单的路由委派（2-4 个专家） | PydanticAI |
| 带循环的质量保证流程 | LangGraph |
| Human-in-the-Loop 审批 | LangGraph |
| 复杂的状态管理和持久化 | LangGraph |
| 快速原型，验证想法 | 手写 / smolagents |
| 只用 OpenAI 模型 | OpenAI Agents SDK 也行 |
...
生产系统的艺术在于找对这根线在哪里。我的经验：**核心路径用硬编码的工作流（高可靠），边缘场景用自由 Agent（高灵活）**。客服系统的退款流程是核心路径，用 LangGraph 固定每一步；用户随口问的杂七杂八问题是边缘场景，用 PydanticAI 自由工具调用兜底。
