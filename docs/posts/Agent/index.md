...
会写 Python 但没碰过 LLM？这个系列用 17 篇文章，把你从"AI Agent 是什么"带到"能独立交付三个生产级 Agent 项目"。以 PydanticAI 为主力框架，LangGraph 做多 Agent 编排，MCP 协议做工具集成标准。每一篇都配 Mermaid 架构图和可运行代码，核心原理额外提供 Manim 动画演示。
...
以下两篇是对照 DataWhale LLM Cookbook 系列后补充的关键专题，强化 Prompt 工程和记忆/评估两个维度。
...
- **Mermaid 图解 + Manim 动画**：ReAct 循环、多 Agent 通信、图状态流转全部可视化
