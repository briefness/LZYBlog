...
> **环境：** Antigravity / Claude Code / Cursor, MCP SDK, Node.js 18+
...
```markdown
# Project Context
Stack: Next.js 15, TailwindCSS, PostgreSQL, Prisma.
...
**3. 效果**
现在的 Agent 可以直接阅读数据库 Schema！
可以直接问：“帮我写一个 SQL 查询过去 7 天活跃度最高的用户”，它会自动读取表结构并生成正确的 SQL。这就是 MCP 的威力。
