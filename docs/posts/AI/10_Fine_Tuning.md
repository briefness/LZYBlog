...
> 
> RAG 就像给参加开卷考试的学生发了教科书。但如果这个学生本身逻辑混乱，书给他也看不懂。
> 这时候就需要 **Fine-tuning (微调)**：把学生关进补习班特训，改变他的大脑连接。
> 本篇核心：**Full Fine-tuning vs PEFT**，以及 **LoRA** 的矩阵分解数学原理。

## 1. 为什么需要微调？

*   **注入特定格式**：比如希望模型总是输出特定的 JSON 格式，Prompt 很难 100% 稳定，微调可以。
*   **注入隐性知识**：比如医疗行业的“潜规则”或语气，这些很难写成文档做 RAG，但可以通过大量病例数据微调学会。
*   **降低成本**：微调一个 7B 的小模型，在特定任务上往往能战胜未微调的 GPT-4。
...
LoRA 的核心洞察是：**模型训练时的权重变化量 $\Delta W$ 是“低秩”的。**
...
*   **压缩了 250 倍！** 这就是为什么一张 4090 也能微调大模型。
...
理论懂了，怎么动手？不要从头写 PyTorch 代码，推荐使用 **LLaMA-Factory**。
...
微调不再是科研机构的专利。
1.  **LoRA** 通过矩阵分解，把大模型微调拉下了神坛。
2.  **LLaMA-Factory** 等工具让微调变成了“配置 config”的工作。
3.  **SFT (Supervised Fine-Tuning)** 是让模型“听话”的关键步骤。

学会了微调，就能打造属于自己的垂直领域专家模型。但怎么知道它练好了没？下一篇，来聊聊 **Evaluation (评测)**。
