...
**显式权衡（Trade-offs）**：
LoRA 通过低秩矩阵分解，将可训练参数从全量（7B 模型对应 4096×4096 维度）压缩到 rank=8 时的约 **6 万**个。消费级 4090 显卡即可完成训练。

代价是：如果底座模型从未见过目标任务领域（比如完全陌生的语言），低秩适配器的容量可能不足以注入足够的领域知识。
...
- 传统的全参数微调会修改几乎所有权重，极易造成原有通用知识的遗忘（旧知识的覆盖风险极高）。
