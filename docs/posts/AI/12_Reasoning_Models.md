...
DeepSeek R1 是怎么练出来的？它没有用昂贵的人工标注数据 (SFT)，而是主要靠 **Reinforcement Learning (RL)**。

### GRPO (Group Relative Policy Optimization)
DeepSeek R1 的核心黑科技。它解决了一个根本问题：**推理过程很难标注**。
让人类老师去检查 AI 几千步的解题步骤哪里错了，极其昂贵且容易出错。

GRPO 采用了 **Outcome Supervision (结果监督)**：
*   **只看结果，不看过程**。
*   做数学题：答案对就是对，错就是错。规则非常简单。
*   **群体验证**：让模型对同一题生成 64 个不同的解题过程。
    *   如果其中 5 个过程算出了正确答案，这 5 个过程就会受到奖励（强化）。
    *   模型会自己去“琢磨”：这 5 个家伙做对了什么？哦，原来是他在第三步多做了一次验算！
    *   慢慢地，模型学会了“自我反思”和“验算”这些行为，因为这些行为能提高算对的概率。

这就是为什么 R1 不需要人类教它怎么思考，它是在几万亿次的自我博弈中，**涌现**出了思考能力。
...
我们在第 03 篇讲过 CoT。但在 R1/o1 这里，CoT 不再是 Prompt 技巧，而是**训练目标**。

**Compute-Optimal Inference**:
以前认为，模型参数越大越聪明（Scaling Law）。
现在发现，**推理时间越长越聪明 (Test-time Scaling)**。
只要给模型足够的时间去 `<think>`，一个小模型 (R1-Distill-7B) 也能在数学/代码任务上击败 GPT-4o。

## 4. 实战：如何使用推理模型？

大部分 API (如 OpenAI, DeepSeek) 会把 `<think>` 内容隐藏掉，只给最终答案。但对于开发者，看到思考通过程非常重要。
...
推理模型的出现，意味着 AI 开始具备了**元认知 (Metacognition)** —— 也就是“对思考的思考”。
