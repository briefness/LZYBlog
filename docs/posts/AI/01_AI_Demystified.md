...
> **不只是黑盒**
> 
> 本文不聊科幻。而是从生物学和数学的交叉点出发，通过 **Perceptron (感知机)** 和 **Gradient Descent (梯度下降)**，推导出 AI 学习的本质。
...
这个 $L$ 就像是地形图上的**高度**。目标是走到山谷最低点（Loss 最小化）。

### Gradient Descent (梯度下降)
身处高山（高 Loss），蒙着眼，怎么下山？
**用脚探路**。
...
*   $\nabla L$: 梯度（导数）。
*   $\eta$: **Learning Rate (学习率)**。步子迈太大容易扯着蛋（震荡），迈太小下山太慢（收敛慢）。
