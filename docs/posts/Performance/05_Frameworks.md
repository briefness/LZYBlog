...
### React Concurrent Mode vs Vue 响应式

React 需要显式迁移。`useTransition`、`useDeferredValue` 是新 API，团队需要学习新的心智模型。Vue 响应式更直观，但灵活性略低——Proxy 的拦截粒度由框架决定，无法精确控制到"只追踪这个属性的某个路径"。

### SolidJS vs Qwik

两者都是细粒度响应式，跳过 Virtual DOM。SolidJS 生态更成熟（社区更大、周边库更多）。Qwik 的 resumability 在 SSR 场景优势更大——它不执行任何 JS 直到用户交互，但 Qwik 生态尚处早期。

### Signals 的优势

无需框架层面的调度器，状态更新精确到 DOM 节点。但代价是：需要所有组件都基于 signals 才能发挥最大效果。迁移成本高——要么全盘重写，要么在混合架构下维护两套状态系统。
